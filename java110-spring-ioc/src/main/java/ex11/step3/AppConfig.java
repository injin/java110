...
public class AppConfig {
    ...
    public Object service(ApplicationContext iocContainer) {
        ...
        Service serviceOriginal = iocContainer.getBean(Service.class);
        ...
        // => 메서드 필터 객체 생성
        MethodFilter filter = new MethodFilter(serviceOriginal);
        
        // => 프록시 객체 만들기
        Service serviceProxy = (Service)Proxy.newProxyInstance(
                // 원래 객체의 클래스 로더
                serviceOriginal.getClass().getClassLoader(),
                
                // 원랙 객체가 구현한 인터페이스들의 타입 정보
